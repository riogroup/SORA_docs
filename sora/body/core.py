...
class Body(BaseBody):
    ...
    diameter : `float`, `int`, `astropy.quantity.Quantity`
        The dimater of the object, in km.
    ...
    These are physical paramaters the user can give to the object. If a query is 
    ...
    def get_pole_position_angle(self, time):
        ...
        position_angle, aperture_angle : `float` array
            Position angle and apeture angle of the object's pole, in degrees.
        """
        time = Time(time)
        pole = self.pole
        if np.isnan(pole.ra):
    ...
    def apparent_magnitude(self, time):
        ...
        ----------
        time :  `str`, `astropy.time.Time`
            Reference time to calculate the object's aparent magnitude.
