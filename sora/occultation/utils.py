...
def positionv(star, ephem, observer, time):
    ...
    Return:
        f, g (float): The orthographic projection of the shadow relative to the 
          observer.
...
def filter_negative_chord(chord, chisquare, step=1, sigma=0):
    ...
    Parameters:
        ...
        step (number, 'exposure'): If a number, it corresponds to the step, in 
          seconds, for each point of the chord path. The step can also be equal 
          to 'exposure'. In this case, the chord path will consider the lightcurve 
          individual times and exptime.
