...
def xy2latlon(x, y, loncen, latcen, time):
    ...
    Returns:
        lon, lat (float): Longitude and Latitude whose projection at loncen, 
          lat results in x, y. (deg).
...
def plot_occ_map(name, radius, coord, time, ca, pa, vel, dist, mag=0, longi=0, **kwargs):
    ...
    Parameters:
        ...
        states (bool): True to plot the state division of the countries. The states 
          of some countries will only be shown depending on the resolution.
        zoom (int, float): Zooms in or out of the map.
        centermap_geo (list): Center the map given coordinates in longitude and 
          latitude. It must be a list with two numbers (Default=None).
        ...
        centerproj (list): Rotates the Earth to show occultation with the center 
          projected at a given longitude and latitude. It must be a list with 
          two numbers.
        labels (bool): Plots text above and below the map with the occultation 
        ...
        parallels (int): Plots lines representing the parallels for given 
          interval (Default=30 deg).
        sites (dict): Plots site positions in map. It must be a python dictionary 
          where the key is  the name of the site, and the value is a list with 
          longitude, latitude, delta_x, delta_y and color. delta_x and delta_y 
          are displacement, in km, from the point of the site in the map and 
          the name. color is the color of the point.
        site_name (bool): If True, it prints the name of the sites given, else 
          it plots only the points.   
        countries (dict): Plots the names of countries. It must be a python 
          ...
          a list with longitude and latitude of the lower left part of the text.
        offset (list): applies an offset to the ephemeris, calculating new CA 
          and instant of CA. It is a pair of delta_RA*cosDEC and delta_DEC.
        mapstyle (int): Define the color style of the map. 1 is the default black 
        ...
        ring (int,float): plots a dashed line representing the location of a 
          ring. It is given in km, from the center.
        rncolor (str): Changes the color of ring lines.
        atm (int,float): plots a dashed line representing the location of an 
          atmosphere. It is given in km, from the center.
        ...
        heights (list): It plots a circular dashed line showing the locations 
          where the observer would observe the occultation at a given height 
          above the horizons. This must be a list.
        ...
        cpoints (int,float): Interval for the small points marking the center 
          of shadow, in seconds (Default: 60).
        ptcolor (str): Change the color of the center points.
        alpha (float): The transparency of the night shade, where 0.0 is full 
        ...
        lncolor (str): Changes the color of the line that represents the limits 
          of the shadow over Earth.
        outcolor (str): Changes the color of the lines that represents the limits 
          of the shadow outside Earth
    ...
    Important:
        Required parameters to plot an occultation map:
        'name', 'radius', 'coord', 'time', 'ca', 'pa', 'vel', and 'dist'.
